...
while cap.isOpened():
    ...
    for (lower, upper) in boundaries:
        ...
        #Initiate node and topics
        dist_Pub = rospy.Publisher('distance', Int16, queue_size=10)   
    	rot_Pub = rospy.Publisher('rotationDeviation', Int16, queue_size=10)
    	rospy.init_node('Position_line', anonymous=True)
    ...
    if lines is not None:
        ...
        #cv2.line(cropped, (int(x0 - vx * w), int(y0 - vy * w)), (int(x0 + vx * w), int(y0 + vy * w)),(0, 0, 255), 1) #Alla linjer typ (RÖD)
        lutning = (vy/vx)
        m = (y0 - x0 * lutning)
        #cv2.line(cropped, (20, lutning*20+m), (300, lutning*300+m),(255,0,0),2) #Alla fitLines (BLÅ)
        win = win + 1
        ...
        cv2.line(cropped, (20, int(xhat[0]*20+mhat[0])), (300, int(xhat[0]*300+mhat[0])),(0,255,0),2)
    ...
    distance = mhat[0]/fakA
    rotationDeviation = fakKurs - math.degrees(math.tan(xhat[0]))
    print('Avstånd från band: ', distance, ' meter. | Grader från optimal kurs: ', rotationDeviation, ' grader. (pos = Höger, neg = Vänster)')
    
    ##Publish the valus to the topics
